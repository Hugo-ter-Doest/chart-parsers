...
ProductionRule.prototype.is_equal_to= function(rule) {
  return ((this.lhs === rule.lhs) &&
    _.isEqual(this.rhs, rule.rhs) &&
    (this.fs ? this.fs.is_equal_to(rule.fs) : true) &&
    (this.head === rule.head));
};
...
ProductionRule.prototype.process_constraint_type = function(constraint, type_lattice) {
  ...
  logger.debug('ProductionRule.process_constraint_type: type is: ' + type.name);
