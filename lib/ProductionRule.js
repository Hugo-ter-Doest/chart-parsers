...
// Walks through the rule's and creates new nodes as necessary
// Returns the deepest node
ProductionRule.prototype.ProcessConstraintRightHandSide = function(c, signature) {
  var rulePtr = this.fs;

  // Process right hand side
  constraint.right.forEach(function(feature) {
    if (rulePtr.features[feature]) { // walk further through the fs
    }
    else { // create a new node
      rulePtr.addFeature(
        feature,
...
ProductionRule.prototype.ProcessConstraint1 = function(c, signature) {
  var rulePtr = this.fs;
  var incomingNode = null;
  var incomingFeature = null;
  ...
  if (c.isList) {
    ...
    if (typeof(c.right) === "string") {
      ...
      var type = signature.typeLattice.getTypeByName(constraint.right);
      if (type) {
        logger.debug('ProductionRule.processConstraint: type is: ' + type.name);
        rulePtr.type = RulePtr.type.LUB(type);
        // TODO: Unify with the type's FS if it exists:
        if (type.fs) {
          // TODO
        }
      }
      else {
        logger.warn('ProductionRule.processConstraint: type not found: ' + constraint.right);
      }
    }
    else { // Right hand side is a feature path
      inComingNode.features[incomingFeature] = this.processConstraintRightHandSide(c, signature);
    }
  }
};
