...
logger.setLevel(settings.loggingLevels.ProductionRule);
...
ProductionRule.prototype.process_constraint = function(constraint, type_lattice) {
  ...
  constraint.left.forEach(function(feature, index) {
    ...
    var new_fs = featureStructureFactory.createFeatureStructure({type: type_lattice.bottom, type_lattice: type_lattice});
    if (current_fs.features[feature]) { 
      // if it already exists unify with new_fs
      current_fs.features[feature] = current_fs.features[feature].unify(new_fs, type_lattice);
    }
    else {
