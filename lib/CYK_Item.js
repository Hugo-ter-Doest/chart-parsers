...
CYK_Item.prototype.is_equal_to = function(item) {
...
CYK_Item.prototype.create_hc_item = function(rule, goal_item, grammar) {
  var new_item = null;
  var new_fs = null;
  var X = rule.rhs[rule.head];
  
  if (grammar.is_headcorner_of(rule.lhs, goal_item.data.nonterminal)) {
    if (GLOBAL.config.UNIFICATION) {
      // Create a feature structure from the CYK item with only the
      // substructure that matches the head of the rule
      new_fs = rule.fs.copy(grammar.type_lattice);
      if (rule.fs.features[X]) {
        if (this.data.fs.features[X]) {
          var fs_with_feature_X = featureStructureFactory.
            createFeatureStructure({type_lattice: grammar.type_lattice});
          fs_with_feature_X.add_feature(X, this.data.fs.features[X], 
            grammar.type_lattice);
          new_fs = new_fs.unify(fs_with_feature_X, grammar.type_lattice);
        }
      }
      else {
        if (this.data.fs.features[X]) {
          new_fs.add_feature(X, this.data.fs.features[X], 
            grammar.type_lattice);
        }
      }
    }
  ...
  items.forEach(function(item) {
    if (typeOf(item) === "goalitem") {
      ...
      rules.forEach(function(rule) {
        var new_item = that.create_hc_item(rule, item, grammar);
        if (new_item) {
      ...
      goals.forEach(function(goal) {
        if (typeOf(goal) === "goalitem") {
          ...
          if ((doubledotteditem.data.rule.rhs[doubledotteditem.data.left_dot-1] === that.data.rule.lhs) &&
            ...
            new_item = doubledotteditem.copy(grammar);
            new_item.recognise_left(that, grammar);
            if (!GLOBAL.config.UNIFICATION || (new_item.data.fs != grammar.type_lattice.top)) {
              nr_items_added += agenda.add_item(new_item, chart);
              logger.debug("CYK_Item.left_complete: " + goal.id + ", " + that.id + ", " + doubledotteditem + " |- " + new_item.id);
            }
            else {
              logger.debug("CYK_Item.left_complete: unification failed: " + goal.id + ", " + that.id + ", " + doubledotteditem);
            }
          }
        }
      });
      ...
      goals.forEach(function(goal) {
        if (typeOf(goal) === "goalitem") {
          ...
          if ((doubledotteditem.data.rule.rhs[doubledotteditem.data.right_dot] === that.data.rule.lhs) &&
              grammar.is_headcorner_of(doubledotteditem.data.rule.lhs, goal.data.nonterminal) ) {
            new_item = doubledotteditem.copy(grammar);
            new_item.recognise_right(that, grammar);
            if (!GLOBAL.config.UNIFICATION || (new_item.data.fs !== grammar.type_lattice.top)) {
              nr_items_added += agenda.add_item(new_item, chart);
              logger.debug("CYK_Item.right_complete: " + goal.id + ", " + doubledotteditem + ", " + that.id + " |- " + new_item.id);
            }
            else {
              logger.debug("CYK_Item.right_complete: unification failed: " + goal.id + ", " + doubledotteditem + ", " + that.id);
            }
          }
        }
      });
