/*
    CYK item class for Head-Corner Parsing
...
CYK_Item.prototype.hc_predict = function(chart, agenda, grammar) {
  ...
  items.forEach(function(item) {
    if (typeOf(item) === "goalitem") {
      ...
      var rules = grammar.get_rules_with_head(that.data.nonterminal);
...
CYK_Item.prototype.left_complete = function (chart, agenda, grammar) {
  ...
  var doubledotteditems = chart.get_items_from(this.data.from);
    ...
    if ((typeOf(doubledotteditem) === "doubledotteditem") && (doubledotteditem.data.left_dot > 0)) {
      goals.forEach(function(goal) {
        if (typeOf(goal) === "goalitem") {
          if ((doubledotteditem.data.rule.rhs[doubledotteditem.data.left_dot-1] === that.data.nonterminal) &&
              grammar.is_headcorner_of(doubledotteditem.data.rule.lhs, goal.data.nonterminal) ) {
            var new_item = new DoubleDottedItem(doubledotteditem.data.rule, doubledotteditem.data.left_dot-1,
                                                doubledotteditem.data.right_dot, that.data.from, doubledotteditem.data.to);
            new_item.children = doubledotteditem.children.slice();
            new_item.children.unshift(that);
            nr_items_added += agenda.add_item(new_item);
            logger.debug("CYK_Item.left_complete: item added: " + new_item.id);
          }
        }
      });
    }
...
CYK_Item.prototype.right_complete = function (chart, agenda, grammar) {
  ...
  doubledotteditems.forEach(function(doubledotteditem) {
    if (typeOf(doubledotteditem) === "doubledotteditem") {
      goals.forEach(function(goal) {
        if (typeOf(goal) === "goalitem") {
          if ((doubledotteditem.data.rule.rhs[doubledotteditem.data.right_dot] === that.data.nonterminal) &&
              grammar.is_headcorner_of(doubledotteditem.data.rule.lhs, goal.data.nonterminal) ) {
            var new_item = new DoubleDottedItem(doubledotteditem.data.rule, doubledotteditem.data.left_dot,
                                                doubledotteditem.data.right_dot+1, doubledotteditem.data.from, that.data.to);
            new_item.children = doubledotteditem.children.slice();
            new_item.children.push(that);
            nr_items_added += agenda.add_item(new_item);
            logger.debug("CYK_Item.right_complete: item added: " + new_item.id);
          }
        }
      });
    }
