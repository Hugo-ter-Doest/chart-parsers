...
CYK_ChartParser.prototype.parse = function(tagged_sentence) {
          ...
          items2.forEach(function(item2) {
            ...
            matching_rules.forEach(function(rule) {
              ...
              if (GLOBAL.config.UNIFICATION) {
                new_fs = rule.fs.copy(that.grammar.signature);
                ...
                if (item1.data.fs.features[rule.rhs[0]]) {
                  if (new_fs.features[rule.rhs[0]]) {
                    ...
                    var fs_with_feature_rhs0 = new TypedFeatureStructure({
                      type: that.grammar.signature.typeLattice.constituent,
                      signature: that.grammar.signature
                    });
                    fs_with_feature_rhs0.addFeature(rule.rhs[0],
                      item1.data.fs.features[rule.rhs[0]], that.grammar.signature);
                    // Unify
                    new_fs = new_fs.unify(fs_with_feature_rhs0, that.grammar.signature);
                  }
                  else {
                    // Add feature
                    new_fs.addFeature(rule.rhs[0], item1.data.fs.features[rule.rhs[0]], that.grammar.signature);
                  }
                }
                ...
                if (item2.data.fs.features[rule.rhs[1]]) {
                  if (new_fs.features[rule.rhs[1]]) {
                    // Create an fs with only feature rule.rhs[0]
                    var fs_with_feature_rhs1 = new TypedFeatureStructure({
                      type: that.grammar.signature.typeLattice.constituent,
                      signature: that.grammar.signature
                    });
                    fs_with_feature_rhs1.addFeature(rule.rhs[1],
                      item2.data.fs.features[rule.rhs[1]], that.grammar.signature);
                    // Unify
                    new_fs = new_fs.unify(fs_with_feature_rhs1, that.grammar.signature);
                  }
                  else {
                    // Add feature
                    new_fs.addFeature(rule.rhs[1], item2.data.fs.features[rule.rhs[1]], that.grammar.signature);
                  }
                }
                if (new_fs !== that.grammar.signature.typeLattice.top) {
  ...
  //logger.debug("Exit CYK_ChartParser.parse: " + JSON.stringify(this.chart,
  // null, 2));
  return this.chart;
};
