...
CYK_ChartParser.prototype.initialise = function(tagged_sentence, func) {
...
CYK_ChartParser.prototype.parse = function(tagged_sentence, func) {
  var that = this;

  logger.debug("Enter CYK_ChartParser.parse: " + JSON.stringify(tagged_sentence, null, 2));
  this.initialise(tagged_sentence, func);
              ...
              if (settings.UNIFICATION) {
                ...
                if (item1.data.fs.features[rule.rhs[0]]) {
                  if (new_fs.features[rule.rhs[0]]) {
                    // Unify
                    new_fs = new_fs.unify(item1.data.fs.features[rule.rhs[0]], that.grammar.type_lattice);
                  }
                  else {
                    // Add feature
                    new_fs.add_feature(item1.data.fs.features[rule.rhs[0]], that.grammar.type_lattice);
                  }
                }
                // Add fs for the first nonterminal
                if (item2.data.fs.features[rule.rhs[1]]) {
                  if (new_fs.features[rule.rhs[1]]) {
                    // Unify
                    new_fs = new_fs.unify(item1.data.fs.features[rule.rhs[1]], that.grammar.type_lattice);
                  }
                  else {
                    // Add feature
                    new_fs.add_feature(item1.data.fs.features[rule.rhs[0]], that.grammar.type_lattice);
