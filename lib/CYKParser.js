...
CYK_ChartParser.prototype.initialise = function(tagged_sentence) {
  ...
  for (var j = 0; j < this.N; ++j) {
    ...
    var term_item = new Item({'lhs': tagged_sentence[j][0], 'rhs': []}, j, j+1);
    // Create a tag item of the form Categorie -> Terminal
    var tag_item = new Item({'lhs': tagged_sentence[j][1], 'rhs': [tagged_sentence[j][0]]}, j, j+1);
    tag_item.add_child(term_item);
    this.chart.add_item(tag_item);
    logger.debug("CYK_ChartParser.initialise: |- " + tag_item.id);
  }
