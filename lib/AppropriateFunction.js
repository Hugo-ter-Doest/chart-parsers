...
AppropriateFunction.prototype.isAppropriateType = function(type1, feature, type2) {
  logger.debug('AppropriateFunction.isAppropriateType');
...
AppropriateFunction.prototype.isAppropriateFeature = function(type1, feature) {
  logger.debug('AppropriateFunction.isAppropriateFeature');
  if (this.applyAppropriateFeatures) {
    logger.debug('AppropriateFunction.isAppropriateFeature: checking (' + type1.name + ' -> ' + feature);
    if (this.appropriateType[type1.name] &&
      this.appropriateType[type1.name][feature]) {
      logger.debug('AppropriateFunction.isAppropriateType: is appropriate: (' + type1.name + ' -> ' + feature);
      return (true);
    }
    else {
      // Recursively try super types of type1
      ...
      if (type1.super_types) {
        type1.super_types.forEach(function (t) {
          isAppropriate = isAppropriate ||
            that.isAppropriateFeature(t, feature);
        });
      }
      return (isAppropriate);
    }
...
// Translates the feature structure associated with type to appropriate
// triples of (type, feature, type)
AppropriateFunction.prototype.processFeatureStructure = function(fs) {
  logger.debug('AppropriateFunction.processFeatureStructure: ' + fs.prettyPrint());
  var that = this;
  Object.keys(fs.features).forEach(function(f) {
    if (!that.appropriateType[fs.type.name]) {
      that.appropriateType[fs.type.name] = {};
    }
    // Add (type, f, type.fs.features[f].type) to the appropriate function
    that.appropriateType[fs.type.name][f] = fs.features[f].type;
    // Add the appropriate features/types of the super types of fs.type
    fs.type.super_types.forEach(function(superType) {
      if (that.appropriateType[superType.name]) {
        Object.keys(that.appropriateType[superType.name]).forEach(function(f2) {
          that.appropriateType[fs.type.name][f2] = that.appropriateType[superType.name][f2];
        });
      }
    });
    // Recurse
    that.processFeatureStructure(fs.features[f]);
  });
};
