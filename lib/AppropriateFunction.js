...
AppropriateFunction.prototype.isAppropriateType = function(type1, feature, type2) {
  ...
  if (this.applyAppropriateTypes) {
    if (type1 && type2 && this.appropriateType[type1.name]) {
      // type 2 may be more specific than the appropriate type
      if (this.appropriateType[type1.name][feature]) {
        logger.debug('AppropriateFunction.isAppropriate' + JSON.stringify(this.appropriateType[type1.name][feature]));
        return (this.appropriateType[type1.name][feature].subsumes(type2));
      }
      else {
        return(false);
      }
...
// Returns true if type can have feature
AppropriateFunction.prototype.isAppropriateFeature = function(type, feature) {
  if (this.applyAppropriateFeatures) {
    if (this.appropriateFeatures[type.name]) {
      return (this.appropriateFeatures[type.name].indexOf(feature) > -1);
    }
    else {
      return (false);
    }
