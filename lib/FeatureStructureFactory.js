...
var TFS = require('./TypedFeatureStructure');

// Constructor
function FeatureStructureFactory(type_lattice) {
  this.type_lattice = type_lattice;
}

// Sets the type lattice that should be used for types of (sub) feature
// structures
FeatureStructureFactory.prototype.set_type_lattice = function(type_lattice) {
  this.type_lattice = type_lattice;
};
...
FeatureStructureFactory.prototype.createFeatureStructure = function(options) {
  // initialise admin for shared substructures
  ...
  // var fs = new TFS(options.type, this.type_lattice);
  var fs = new TFS(options.dag, map_label_to_node, this.type_lattice);
  // if (options.dag) {
  //   fs.process_dag(dag, this.type_lattice);
  // }
  // create a graph from it
  fs.set_shared_nodes(map_label_to_node, this.type_lattice);
  if (options.type) {
    logger.debug('FeatureStructureFactory.createFeatureStructure ' + options.type.name);
