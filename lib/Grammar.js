...
Grammar.prototype.computeLCRelation = function() {
  var that = this;
  ...
  this.production_rules.forEach(function(rule) {
    ...
    if (that.isNonterminal(rule.rhs[0])) {
...
Grammar.prototype.rulesWithLeftmostDaughter = function (B) {
  rules = [];
...
Grammar.prototype.isLeftcornerOf = function(B, C) {
  if (this.lc[C]) {
    return(this.lc[C][B]);
  }
  else {
...
Grammar.prototype.isHeadcornerOf = function(A, B) {
  if (this.hc[B]) {
    return(this.hc[B][A]);
  }
  else {
...
Grammar.prototype.getRulesWithHead = function(H) {
  var res = [];
...
Grammar.prototype.computeHCRelation = function() {
  var that = this;
...
Grammar.prototype.prettyPrint = function(debug) {
  var result = '';
  var newline = '\n';
  var that = this;
  this.production_rules.forEach(function(rule) {
    result += rule.prettyPrint(that.signature, debug) + newline;
  });
  return(result);
};
