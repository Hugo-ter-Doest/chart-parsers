...
function Chart(N) {
  ...
  this.on('itemAddedToChart', settings.event_function);
}
...
Chart.prototype.add_item = function(item) {
  ...
  if (nr_items_added > 0) {
    // emit an event
    this.emit('itemAddedToChart', 'itemAddedToChart', item);
  }
...
Chart.prototype.full_parse_items = function(A, item_type) {
  var that = this;
  var items = [];
  
  logger.debug("Enter Chart.full_parse_items(" + A + ")");
  this.get_complete_items_from_to(0, this.N).forEach(function(item) {
    logger.debug('Exit Chart.full_parse_items: type of item: ' + typeOf(item)); 
