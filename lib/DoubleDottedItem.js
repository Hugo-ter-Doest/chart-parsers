    ...
    items1.forEach(function(item1) {
      if (typeOf(item1) === "goalitem") { 
        ...
        items2.forEach(function(item2) {
          logger.debug("DoubleDottedItem.left_complete: trying: " + item1.id +", " + item2.id + ", " + that.id);
          if ((typeOf(item2) === "cyk_item") &&
              (that.data.rule.rhs[that.data.left_dot-1] === item2.data.nonterminal) &&
              grammar.is_headcorner_of(that.data.rule.lhs, item1.data.nonterminal)) {
            var new_item = new DoubleDottedItem(that.data.rule, that.data.left_dot-1, that.data.right_dot, item2.data.from, that.data.to);
  ...
  if (this.data.right_dot < this.data.rule.rhs.length) {
    ...
    items1.forEach(function(item1) {
      if (typeOf(item1) === "goalitem") { 
        ...
        items2.forEach(function(item2) {
          logger.debug("DoubleDottedItem.right_complete: trying: " + item1.id +", " + that.id + ", " + item2.id);
          if (typeOf(item2) === "cyk_item") {
            if ((that.data.rule.rhs[that.data.right_dot] === item2.data.nonterminal) &&
                grammar.is_headcorner_of(that.data.rule.lhs, item1.data.nonterminal)) {
              var new_item = new DoubleDottedItem(that.data.rule, that.data.left_dot, that.data.right_dot+1, that.data.from, item2.data.to);
              new_item.children = that.children.slice();
