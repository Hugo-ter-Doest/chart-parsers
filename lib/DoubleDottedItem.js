...
DoubleDottedItem.prototype.copy = function (grammar) {
  ...
  var new_item = itemFactory.createItem({
    ...
    'from': this.data.from,
    'to': this.data.to,
    'fs': GLOBAL.config.UNIFICATION ? this.data.fs.copy(grammar.type_lattice) : null
  });
  ...
  if (GLOBAL.config.UNIFICATION) {
    if (this.data.fs.features[B]) {
      if (cyk_item.data.fs.features[B]) {
        ...
        var fs_with_feature_B = new TypedFeatureStructure({type_lattice: grammar.type_lattice});
        fs_with_feature_B.addFeature(B, cyk_item.data.fs.features[B], grammar.type_lattice);
      ...
      if (cyk_item.data.fs.features[B]) {
        ...
        this.data.fs.addFeature(B, cyk_item.data.fs.features[B], grammar.type_lattice);
      }
    }
  }
