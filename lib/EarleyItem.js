...
// Compares two items
EarleyItem.prototype.isEqualTo = function(item) {
      ...
      if (GLOBAL.config.UNIFICATION) {
        ...
        if (new_fs.features[B]) {
          if (that.data.fs.features[B]) {
            ...
            fs_with_feature_B.addFeature(B, that.data.fs.features[B], grammar.type_lattice);
            logger.debug('EarleyItem.predictor: unifying: \n' + 
                new_fs.prettyPrint() + '\nwith \n' +
                fs_with_feature_B.prettyPrint());
            new_fs = new_fs.
          ...
          if (that.data.fs.features[B]) {
            new_fs.addFeature(B, that.data.fs.features[B], grammar.type_lattice);
          }
        }
