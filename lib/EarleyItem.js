...
EarleyItem.prototype.copy = function() {
  logger.debug("Enter EarleyItem.copy: " + this);
  var new_item = new EarleyItem(this.data.rule, this.data.dot, this.data.from, this.data.to);
  new_item.children = this.children.slice();
  logger.debug("Exit EarleyItem.copy: " + new_item);
  return(new_item);
};
...
EarleyItem.prototype.completer = function(chart, grammar) {
  ...
  console.log("EarleyItem.completer: added " + nr_items_added + " items");
  return(nr_items_added);
};
