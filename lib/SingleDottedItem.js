...
function SingleDottedItem(rule, dot, from, to) {
...
SingleDottedItem.prototype.set_children = function(children) {
  this.children = children;
}

SingleDottedItem.prototype.add_child = function(child) {
  this.children.push(child);
}

// Checks if an item is incomplete
SingleDottedItem.prototype.is_incomplete = function () {
...
SingleDottedItem.prototype.is_complete = function () {
...
SingleDottedItem.prototype.predictor = function(chart, grammar) { 
  var that = this;
  ...
  if (this.is_incomplete() && grammar.is_nonterminal(this.data.rule.rhs[this.data.dot])) {
    ...
    rules_with_lhs_B.forEach(function(rule) {
      var newitem = new SingleDottedItem(rule, 0, this.data.to, this.data.to);
...
SingleDottedItem.prototype.lc_predictor = function(chart, grammar) {
  that = this;
  var nr_items_added = 0;
  ...
  if (this.is_complete()) {
    ...
    grammar.rules_with_leftmost_daughter(this.data.rule.lhs).forEach(function(rule) {
      ...
      chart.get_items_to(item.data.from).forEach(function(item2) {
        if (item2.is_incomplete() && grammar.is_leftcorner_of(rule.lhs, item2.data.rule.rhs[item2.data.dot])) {
          var newitem = new SingleDottedItem(rule, 1, that.data.from, that.data.to);
...
SingleDottedItem.prototype.completer = function(chart, grammar) {
  var that = this;
  var nr_items_added = 0;

  console.log('Completer: ' + item.id);
  if (this.is_complete()) {
    var items = chart.get_items_to(item.data.from);
    items.forEach(function(item2) {
      if (item2.is_incomplete() && (that.data.rule.lhs === item2.data.rule.rhs[item2.data.dot])) {
        var new_item = new SingleDottedItem(item2.data.rule, item2.data.dot + 1, item2.data.from, that.data.to);
...
module.exports = SingleDottedItem;