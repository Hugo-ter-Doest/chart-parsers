  ...
  items.forEach(function(item) {
    if (typeOf(item) === "doubledotteditem") {
      if (grammar.is_headcorner_of(item.data.rule.lhs, that.data.nonterminal)) {
        for (var i = that.data.from; i <= that.data.to; i++) {
          for (var j = i; i < that.data.to; j++) {
...
GoalItem.prototype.right_predict = function (chart, agenda, grammar) {
  ...
  items.forEach(function(item) {
    if (typeOf(item) === "doubledotteditem") {
      if (grammar.is_headcorner_of(item.data.rule.lhs, that.data.nonterminal)) {
        for (var i = that.data.i; i <= that.data.to; i++) {
          for (var j = i; i < that.data.to; j++) {
    ...
    items1.forEach(function(item1) {
      if (typeOf(item1) === "cyk_item") {
        items2.forEach(function(item2) {
          if (typeOf(item2) === "doubledotteditem") { 
            if ((item2.data.rule.lhs[item2.data.left_dot] === item1.data.nonterminal) && 
      ...
      if (typeOf(item1) === "doubledotteditem") {
        items2.forEach(function(item2) {
          if (typeOf(item2) === "cyk_item") {
            if ((item1.data.rule.lhs[item1.data.right_dot] === item2.data.nonterminal) && 
