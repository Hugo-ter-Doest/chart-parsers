...
GoalItem.prototype.hc_predict = function(chart, agenda, grammar) {
  ...
  items.forEach(function(item) {
    if (typeOf(item) === "cyk_item") {
      ...
      var rules = grammar.get_rules_with_head(item.data.nonterminal);
  ...
  if (this.data.from === this.data.to) {
    grammar.production_rules.forEach(function(rule) {
      if ((rule.rhs.length === 0) && grammar_is_headcorner_of(rule.lhs, that.nonterminal)) {
        var new_item = new CYK_item(rule.lhs, that.data.from, that.data.to);
          nr_items_added += agenda.add_item(new_item);
        }
    });
  }
...
GoalItem.prototype.left_predict = function (chart, agenda, grammar) {
  ...
  items.forEach(function(item) {
    if (typeOf(item) === "doubledotteditem") {
      ...
      if (typeOf(item1) === "cyk_item") {
        items2.forEach(function(item2) {
          if ((typeOf(item2) === "doubledotteditem") && (item2.data.left_dot > 0)) {
            logger.debug("GoalItem.left_complete: combine: " + that.id + ", " + item1.id  + ", " + item2.id);
            if ((item2.data.rule.rhs[item2.data.left_dot-1] === item1.data.nonterminal) && 
                grammar.is_headcorner_of(item2.data.lhs, that.data.nonterminal)) {
              var new_item = new DoubleDottedItem(item2.data.rule, item2.data.left_dot-1, item2.data.right_dot, that.data.from, that.data.to);
              new_item.set_children(item2.children.slice());
              new_item.children.unshift(item1);
              nr_items_added += agenda.add_item(new_item);
              logger.debug("GoalItem.left_complete: item added: " + new_item.id);
            }
          }
        });
      }
      ...
      if (typeOf(item1) === "doubledotteditem") {
        items2.forEach(function(item2) {
          if (typeOf(item2) === "cyk_item") {
            logger.debug("GoalItem.right_complete: combine: " + that.id + ", " + item1.id  + ", " + item2.id);
            if ((item1.data.rule.rhs[item1.data.right_dot] === item2.data.nonterminal) && 
                grammar.is_headcorner_of(item1.data.lhs, that.data.nonterminal)) {
              var new_item = new DoubleDottedItem(item1.data.rule, item1.data.left_dot, item1.data.right_dot+1, that.data.from, that.data.to);
              new_item.children = item1.children.slice();
              new_item.children.push(item2);
              logger.debug("GoalItem.right_complete: created item: " + new_item.id);
              nr_items_added += agenda.add_item(new_item);
            }
          }
        });
