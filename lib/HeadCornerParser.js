...
HeadCornerChartParser.prototype.initialise_goals = function(tagged_sentence) {
  var that = this;
  var new_item;
  var nr_items_added = 0;
  var i,j;

  logger.debug("Enter HeadCornerChartParser.initialise_goals");
  ...
  for (i = 0; i <= this.N; i++) {
    for (j = i; j <= this.N; j++) {
      var new_item = itemFactory.createItem({
        'type': 'Goal',
        'nonterminal': this.grammar.get_start_symbol(),
        'from': i,
        'to': j
      });
      nr_items_added += this.agenda.addItem(new_item, this.chart);
      logger.debug("HeadCornerChartParser.initialise: |- " + new_item.id);
    }
  }
  logger.debug("Enter HeadCornerChartParser.initialise_goals: number of items added: " + nr_items_added);
  return(nr_items_added);
};

HeadCornerChartParser.prototype.initialise_sentence_cfg = function(tagged_sentence) {
  var nr_items_added = 0;
...
HeadCornerChartParser.prototype.initialise_sentence_ug = function(tagged_sentence) {
  var nr_items_added = 0;
  ...
  for (var i = 0; i < this.N; i++) {
    ...
    for (var j = 1; j < tagged_sentence[i].length; j++) {
      ...
      logger.debug('HeadCornerChartParser.initialise_sentence_ug: category ' + category);
      var new_fs = featureStructureFactory.createFeatureStructure({'type_lattice': this.grammar.type_lattice});
      new_fs.add_feature(category, tagged_sentence[i][j], this.grammar.type_lattice);
      var tag_item = itemFactory.createItem({
        'type': 'CYK',
        'rule': {'lhs': category, 'rhs': [this.tagged_sentence[i][0]]},
        'from': i,
