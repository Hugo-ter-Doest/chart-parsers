...
ChartParser.prototype.initialise_sentence_cfg = function(tagged_sentence, func) {
...
// Adds items to the chart for each word in the tagged sentence
// tagged_sentence[i] is an array of feature structures
ChartParser.prototype.initialise_sentence_ug = function(tagged_sentence, func) {
  var nr_items_added = 0;
  ...
  for (var i = 0; i < this.N; i++) {
    ...
    // Add tag items
    for (var j = 1; j < tagged_sentence[i].length; j++) {
      // for readability take the category from the feature structure
      var category = this.tagged_sentence[i][j].features[category];
      var tag_item = itemFactory.createItem({
        'type': 'Earley',
...
ChartParser.prototype.initialise = function(tagged_sentence, func) {
  ...
  if (UNIFICATION) {
    nr_items_added += this.initialise_sentence_ug(tagged_sentence, func);
  }
  else {
    nr_items_added += this.initialise_sentence_cfg(tagged_sentence, func);
  }
