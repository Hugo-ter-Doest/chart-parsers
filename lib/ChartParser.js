...
ChartParser.prototype.initialiseSentenceCFG = function(tagged_sentence) {
...
ChartParser.prototype.initialiseSentenceUG = function(tagged_sentence) {
  var nr_items_added = 0;
  ...
  for (var i = 0; i < this.N; i++) {
    ...
    for (var j = 1; j < tagged_sentence[i].length; j++) {
      ...
      new_fs.addFeature(category, tagged_sentence[i][j], this.type_lattice);
      var tag_item = itemFactory.createItem({
        'type': 'Earley',
        'rule': {'lhs': category, 'rhs': [this.tagged_sentence[i][0]]},
        'dot': 1,
...
ChartParser.prototype.initialiseGoals = function(tagged_sentence) {
  var nr_items_added = 0;
  ...
  rules.forEach(function(rule) {
    ...
    nr_items_added += that.chart.addItem(new_item);
  });
  return(nr_items_added);
};
