  ...
  function copyUnifact0(fs, signature, stack) {
    ...
    if (fs) {
      if (stack.indexOf(fs.id) > -1) {
        logger.debug('TypedFeatureStructure.copyUnifact: cycle detected ' + fs.id);
        ...
        logger.debug('TypedFeatureStructure.copyUnifact: create new' +
          ' unifact node: ' + fs.id + ' -> ' + fs.unifact.id);
        fs.unifact_mark = current_generation;
        logger.debug('TypedFeatureStructure.copyUnifact: unifact returned ' + fs.unifact.id);
        return (fs.unifact);
      }
      var newStack = stack.slice();
      newStack.push(fs.id)
      ...
      if (fs.unifact_mark === current_generation) {
        logger.debug('TypedFeatureStructure.copyUnifact: unifact_mark equals' +
