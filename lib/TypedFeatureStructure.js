...
TypedFeatureStructure.prototype.unifiable = function(fs, signature) {
  ...
  function unifiableListOfCrossRefs(fs1, fs2) {
    var unifiable = true;
      ...
      if (unifiable) {
        fs1.listOfCrossRefs.forEach(function (ref, index) {
          if (ref.type === signature.typeLattice.string) {
            ...
            unifiable = unifiable && unifiable0(ref, fs2.listOfCrossRefs[index], signature);
          }
        });
      }
      if (unifiable) {
  ...
  function unifiable0(a, b, signature) {
    ...
    if (fs1.aux_type === signature.typeLattice.string) {
      ...
      if (fs1.aux_type === signature.typeLattice.list) {
        return(unifiableListOfCrossRefs(fs1, fs2));
      }
      else { // Ordinary node
