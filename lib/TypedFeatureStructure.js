...
TypedFeatureStructure.prototype.addFeature = function(feature, fs, signature) {
  ...
  // appropriate
  if (fs.coreference || signature.isAppropriate(this.type, feature, fs.type)) {
...
// Returns true if the feature structure is totally well typed with respect
// to the signature.
TypedFeatureStructure.prototype.isTotallyWellTyped = function(signature) {

};

// This unifies the feature structure associated with this.type with the
// feature structures of its super types. This assures that valid paths are
// inheritec from super types.
      ...
      if (fs.features) {
        Object.keys(fs.features).forEach(function (f) {
          if (fs.features[f].type.fs) {
            fs.features[f] = fs.features[f].unify(fs.features[f].type.fs, signature);
          }
          logger.debug('problem with feature ' + f);
