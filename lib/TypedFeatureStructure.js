...
TypedFeatureStructure.prototype.unifiable = function(fs, signature) {
  ...
  function unifiable0(a, b, signature) {
      ...
      if (cycleDetect[a.id] && cycleDetect[a.id][b.id]) {
        return(true);
      }
      if (cycleDetect[b.id] && cycleDetect[b.id][a.id]) {
        return(true);
      }

      // Register this pair of nodes
      if (!cycleDetect[a.id]) {
        cycleDetect[a.id] = {};
      }
      cycleDetect[a.id][b.id] = true;
  ...
  function findCycles0(fs, stack) {
    Object.keys(fs.features).forEach(function(f) {
      ...
      if (stack.indexOf(fs.features[f].id) > -1) {
        ...
        stack.push(fs.id);
        // Recurse depth first
        findCycles0(fs.features[f], stack);
      }
    });
  }
};
...
TypedFeatureStructure.prototype.copyUnifact = function(signature) {
  ...
  function copyUnifact0(fs, signature) {
    if (fs) {
      ...
      if (need_to_copy) {
        if (fs.unifact === null) {
          logger.debug('TypedFeatureStructure.copyUnifact: create new unifact node for node: ' + fs.id);
        ...
        logger.debug('TypedFeatureStructure.copyUnifact: exit: no need to' +
          ' copy: current feature structure returned: ' + fs.id);
        return (fs);
      }
    }
    else {
