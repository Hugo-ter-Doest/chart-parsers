...
TypedFeatureStructure.prototype.unifiableListOfCrossRefs =
  ...
  if (fs1.listOfCrossRefs && fs2.listOfCrossRefs) {
    ...
    if (unifiable) {
      unifiable = unifiable && fs1.listOfCrossRefs.every(function (ref, index) {
...
TypedFeatureStructure.prototype.completeFS1 = function(signature) {
  ...
  var fs = this;
  Object.keys(fs.features).forEach(function (f) {
    fs.features[f] = completeFS0(fs.features[f], signature, [f]);
  });
  return (fs);
  ...
  function completeFS0(fs, signature, path) {
    if (!visited[fs.id]) {
      ...
      if (fs.type.fs) {
        ...
        fs = fs.unify(copyWithPrefix, signature);
        var newFS = fs.followPath(path);
        Object.keys(newFS.features).forEach(function (f) {
          var newPath = path.slice();
          newPath.push(f);
          newFS.features[f] = completeFS0(newFS.features[f], signature, newPath);
        });
        return (newFS);
      }
      else { // No FS associated with the type
        Object.keys(fs.features).forEach(function (f) {
          var newPath = path.slice();
          newPath.push(f);
          fs.features[f] = completeFS0(fs.features[f], signature, newPath);
        });
        return (fs);
      }
    }
    else { // Already completed this node
      return(fs);
    }
  }
};
