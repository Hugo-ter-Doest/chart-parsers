...
TypedFeatureStructure.prototype.setSharedNodes = function(map_label_to_node, signature) {
  ...
  Object.keys(fs.features).forEach(function(feature) {
    ...
    if (fs.features[feature].coreference) {
      ...
      fs.features[feature].setSharedNodes(map_label_to_node, signature);
    }
  });
};
...
TypedFeatureStructure.prototype.setFeatures = function(features, signature) {
  //if (signature.hasCompleteAndAppropriateFeatures(this.type, features)) {
    var that = this;
  ...
  //}
  //else {
  //  logger.warn('TypedFeatureStructure.setFeatures: type ' + this.type.name +
  //    ' and features: ' + JSON.stringify(features, null, 2));
  //}
};
...
TypedFeatureStructure.prototype.addFeature = function(feature, fs, signature) {
  // If fs is a temporary coreference TSF then we do not check if it is
  // appropriate
  //if (fs.coreference || signature.isAppropriate(this.type, feature,
  // fs.type)) {
...
TypedFeatureStructure.prototype.unify = function(fs, signature) {
  ...
  if (this.unifiable(fs, signature)) {
    ...
    var result = signature.typeLattice.top;
    return(result);
  }
};
...
TypedFeatureStructure.prototype.unifiable = function(fs, signature) {
  ...
  function feature_is_unifiable(feature) {
    ...
    if (fs1.features[feature]) {
      var unifiable = fs1.features[feature].unifiable(fs2.features[feature], signature);
      logger.debug('TypedFeatureStructure.unifiable: checking feature: ' + feature + 
...
TypedFeatureStructure.prototype.copyUnifact = function(signature) {
  ...
  var copies = {};
  Object.keys(fs.features).forEach(function(feature) {
    copies[feature] = fs.features[feature].copyUnifact(signature);
    need_to_copy = need_to_copy || (copies[feature] !== fs.features[feature]);
  });
  Object.keys(fs.aux_features).forEach(function(feature) {
    copies[feature] = fs.aux_features[feature].copyUnifact(signature);
  ...
  if (need_to_copy) {
    if (fs.unifact === null) {
      ...
      fs.unifact = new TypedFeatureStructure({type: signature.typeLattice.bottom});
      if (fs.aux_type) {
...
TypedFeatureStructure.prototype.isEqualTo = function(fs) {
  ...
  if (this.type !== fs.type) {
    logger.debug('TypedFeatureStructure.is_equal_to: false' + JSON.stringify(this.type) + ' !== ' + JSON.stringify(fs.type));
    return(false);
  }
  // compare features
...
TypedFeatureStructure.prototype.copy = function(signature) {
  old_id_to_new_node = {};
  return(this.copy0(signature));
};

// The actual copying is done in this method
TypedFeatureStructure.prototype.copy0 = function(signature) {
  var result = new TypedFeatureStructure({type: this.type});
  var that = this;
  Object.keys(this.features).forEach(function(feature) {
    if (!old_id_to_new_node[that.features[feature].id]) {
      result.features[feature] = that.features[feature].copy0(signature);
...
TypedFeatureStructure.prototype.inheritFromSuperTypes = function(signature) {
  ...
  this.type.super_types.forEach(function (superType) {
    if (superType.fs) {
      fs = fs.unify(superType.fs, signature);
    }
  });
  return(fs);
};
...
TypedFeatureStructure.prototype.completeFS = function(signature) {
  var fs = this;
  if (fs.type.fs) {
    fs = fs.unify(fs.type.fs, signature);
  }
  if (fs.features) {
    Object.keys(fs.features).forEach(function (f) {
      fs.features[f] = fs.features[f].completeFS(signature);
    });
  }
  return(fs);
};

// Unifies the feature structure with the feature structures assigned to
// the types of its nodes. This used after (the constraints of ) a production
// rule has been parsed into a feature structure.
TypedFeatureStructure.prototype.unifyWithTypes = function(signature) {
  logger.debug('TypedFeatureStructure.unifyWithTypes');
  var fs = this;
  //if (fs.type.fs) {
  //  fs = fs.unify(fs.type.fs, signature);
  //}
  if (fs.features) {
    Object.keys(fs.features).forEach(function (f) {
      if (fs.features[f].type.fs) {
        fs.features[f] = fs.features[f].unify(fs.features[f].type.fs,
          signature);
      }
      logger.debug('TypedFeatureStructure.unifyWithTypes:' +
        ' processing feature ' + f + fs.features[f].prettyPrint());
      //fs.features[f] = fs.features[f].inheritFromSuperTypes(signature);
    });
  }
  return(fs);
};
...
TypedFeatureStructure.prototype.resetPrintedFlags = function() {
  var cycleDetect = {};
  resetPrintedFlags0(this);

  function resetPrintedFlags0(fs) {
    if (fs && !cycleDetect[fs.id]) {
      fs.printed = false;
      cycleDetect[fs.id] = true;
      Object.keys(fs.features).forEach(function (feature) {
        resetPrintedFlags0(fs.features[feature]);
      });
    }
  }
};
...
TypedFeatureStructure.prototype.resetIncomingCounters = function() {
  var cycleDetect = {};
  resetIncomingCounters0(this);

  function resetIncomingCounters0(fs) {
    if (fs && !cycleDetect[fs.id]) {
      fs.incoming = {};
      cycleDetect[fs.id] = true;
      Object.keys(fs.features).forEach(function (feature) {
        resetIncomingCounters0(fs.features[feature]);
      });
    }
  }
};
...
TypedFeatureStructure.prototype.prettyPrint0 = function(indent) {
  ...
  logger.debug('TypedFeatureStructure.prettyPrint0: features: ' + this);
