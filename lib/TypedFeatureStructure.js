  ...
  function unifiable0(a, b, signature) {
    ...
    if (fs1.aux_type === signature.typeLattice.top) {
      logger.debug('TypedFeatureStructure.unifiable: types cannot be' +
        ' unified: ' + fs1.type.prettyPrint(signature) + ' ' + fs2.type.prettyPrint(signature));
      return (false);
    }
...
TypedFeatureStructure.prototype.isEqualTo = function(fs, signature) {
  ...
  function isEqualTo0(fs1, fs2) {
    ...
    if (fs1.type !== fs2.type) {
      logger.debug('TypedFeatureStructure.is_equal_to: false ' + fs1.type.prettyPrint(signature) + ' !== ' + fs2.type.prettyPrint(signature));
      return (false);
    }
