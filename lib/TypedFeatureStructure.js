...
TypedFeatureStructure.prototype.unifiable = function(fs, signature) {
  ...
  function unifiable0(a, b, signature) {
    ...
    if (fs1.aux_type.name === GLOBAL.config.typeOfLexicalString) {
      // compare lexical strings
      if (fs1.lexicalString !== fs2.lexicalString) {
        return(false);
      }
    }
    if (fs1.aux_type.name === GLOBAL.config.typeOfListOfCrossRefs) {
      // compare array of references to lexical strings
      var equal = (fs1.listOfCrossRefs.length === fs2.listOfCrossRefs.length);
      if (equal) {
        fs1.listOfCrossRefs.forEach(function (ref, index) {
          // Recurse to check nested references of cross references strings?
          if (ref.type.name === GLOBAL.config.typeOfLexicalString) {
            // String cross reference
            equal = equal && (ref.lexicalString === fs2.listOfCrossRefs[index].lexicalString);
          }
          else {
            // Normal cross reference
            equal = equal & unifiable0(ref, fs2.listOfCrossRefs[index], signature);
          }
        });
      }
      if (!equal) {
        logger.debug('TypedFeatureStructure.unifiable: lexical strings cannot' +
          ' be unified');
        return(false);
      }
    }
...
TypedFeatureStructure.prototype.copyUnifact = function(signature) {
  ...
  function copyUnifact0(fs, signature, stack) {
    ...
    if (fs.unifact.type.name === GLOBAL.config.typeOfLexicalString) {
      fs.unifact.lexicalString = fs.lexicalString;
    }

    // Copy list of references to phonstrings
    if (fs.unifact.type.name === GLOBAL.config.typeOfListOfCrossRefs) {
      fs.unifact.listOfCrossRefs = fs.listOfCrossRefs.slice();
    }
  ...
  function copy0(fs, signature) {
    if (!old_id_to_new_node[fs.id]) {
      ...
      if (fs.type.name === GLOBAL.config.typeOfListOfCrossRefs) {
        result.listOfCrossRefs = fs.listOfCrossRefs.slice();
      }
    ...
    if (fs) {
      ...
      if (!Object.keys(fs.features).length) {
        ...
        if (fs.type.name === GLOBAL.config.typeOfLexicalString) {
          ...
          if (fs.type.name === GLOBAL.config.typeOfListOfCrossRefs) {
            // Print the lexical strings of the referred to nodes
            result += '<';
            fs.listOfCrossRefs.forEach(function(ref) {
              result += (debug ? ref.getLabel() : '');
              if (ref.type.name === GLOBAL.config.typeOfLexicalString) {
                result += '\"' + ref.lexicalString + '\", ';
              }
              else {
                result += ref.getLabel() + ', ';
              }
