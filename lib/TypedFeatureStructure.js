...
function TypedFeatureStructure(options) {
  ...
  if (!this.type) {
    this.type = options.type_lattice.bottom;
  }
  this.features = {};
  this.aux_type = null;
  this.aux_features = {};
  ...
  if (options.features) {
    this.setFeatures(options.features, options.type_lattice);
  }
}
...
TypedFeatureStructure.prototype.unify = function(fs, type_lattice) {
  ...
  if (this.unifiable(fs, type_lattice)) {
    ...
    var result = new TypedFeatureStructure({type: type_lattice.top});
    return(result);
  }
};
...
TypedFeatureStructure.prototype.copyUnifact = function(type_lattice) {
  ...
  if (need_to_copy) {
    if (fs.unifact === null) {
      ...
      fs.unifact = new TypedFeatureStructure({type: type_lattice.bottom});
      if (fs.aux_type) {
