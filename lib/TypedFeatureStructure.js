    ...
    if ((fs1.aux_type === signature.typeLattice.list) ||
      ...
      function featureIsUnifiable(feature) {
        ...
        if (fs1.features[feature]) {
          ...
          if ((fs2.features[feature].type === signature.typeLattice.list) ||
            (fs2.features[feature].type === signature.typeLattice.concatlist)) {
      ...
      Object.keys(fs1.features).forEach(function(feature) {
        if (!fs2.features[feature]) {
          if ((fs1.features[feature].type === signature.typeLattice.list) || (fs1.features[feature].type === signature.typeLattice.concatlist)){
    ...
    if ((fs.unifact.type === signature.typeLattice.list) ||
      ...
      if (fs.aux_listOfCrossRefs) {
        ...
        if (fs.listOfCrossRefs) {
          logger.debug('TypedFeatureStructure.copyUnifact0: copying' +
            ' concat list');
          var list = [];
...
TypedFeatureStructure.prototype.prefixPath = function(signature, path) {
  ...
  if (path.length === 0) {
    ...
    newFS.addFeature(feature, this.prefixPath(signature, newPath));
    return(newFS);
  }
};
          ...
          if (fs.type === signature.typeLattice.list) {
            ...
            if (fs.type === signature.typeLattice.concatlist) {
              ...
              if (fs.listOfCrossRefs) {
                ...
                var nrPrinted = 0;
                fs.listOfCrossRefs.forEach(function (ref) {
                  // If ref refers to a list then print the elements as part of
                  // the concatenation
                  if ((ref.type === signature.typeLattice.list)  && (ref.listOfCrossRefs)) {
                    ref.listOfCrossRefs.forEach(function(ref1) {
                      if (ref1.printed) {
                        result += ref1.getLabel() + ', ';
                      }
                      else { // Print the referred fs
                        result += prettyPrint0(ref1, indent + 1) + ',\n';
                      }
                      nrPrinted++;
                    });
                  }
