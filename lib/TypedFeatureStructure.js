...
function TypedFeatureStructure(dag, map_label_to_node, type_lattice) {
  this.features = {};
  // this.type = type;
  this.type = type_lattice.get_type_by_name('BOTTOM');
  this.aux_type = null;
  this.aux_features = {};
  this.forward = null;
  ...
  if (typeof(dag) === 'string') {
    ...
    Object.keys(dag).forEach(function(feature) {
      if (feature.indexOf(':') >= 0) {
        ...
        // fs.process_dag(dag[feature], map_label_to_node, type_lattice);
        var fs = new TypedFeatureStructure(dag[feature], map_label_to_node, type_lattice);
        fs.label = label;
        ...
        if (feature === 'type') {
          ...
          // var fs = TypedFeatureStructure(type_lattice.bottom, type_lattice);
          // fs.process_dag(dag[feature], map_label_to_node, type_lattice);
          var fs = new TypedFeatureStructure(dag[feature], map_label_to_node, type_lattice);
...
TypedFeatureStructure.prototype.copy_unifact = function(type_lattice) {
  ...
  if (need_to_copy) {
    if (fs.unifact === null) {
      ...
      fs.unifact = new TypedFeatureStructure(null, null, type_lattice);
      if (fs.aux_type) {
