...
TypedFeatureStructure.prototype.unifiable = function(fs, signature) {
  ...
  function unifiable0(a, b, signature) {
        ...
        function feature_is_unifiable(feature) {
          ...
          if (fs1.features[feature] && fs2.features[feature]) {
...
TypedFeatureStructure.prototype.unifyWithTypes = function(signature) {
  ...
  var fs = this;
  //var cycleDetect = {};
  //return(unifyWithTypes0(this, signature));

  //function unifyWithTypes0(fs, signature) {
    //if (!cycleDetect[fs.id]) {
      //cycleDetect[fs.id] = true;
      if (fs.features) {
        Object.keys(fs.features).forEach(function (f) {
  ...
  function prettyPrint0(fs, indent) {
    ...
    if (fs) {
      ...
      if (!Object.keys(fs.features).length) {
        ...
        result += fs.type.prettyPrint(); // + ' (incoming: ' +
