...
TypedFeatureStructure.prototype.unifiable = function(fs, signature) {
  ...
  function unifiable0(a, b, signature) {
    if (!pairOfNodesSeen[a.id]) {
      pairOfNodesSeen[a.id] = {};
    }
    if (!pairOfNodesSeen[b.id]) {
      pairOfNodesSeen[b.id] = {};
    }
    if (!pairOfNodesSeen[a.id][b.id] && !pairOfNodesSeen[b.id][a.id]) {
      pairOfNodesSeen[a.id][b.id] = true;
      var fs1 = a.dereference();
      var fs2 = b.dereference();
      pairOfNodesSeen[fs1.id][fs2.id] = true;
      ...
      logger.warn('TypedFeatureStructure.unifiable: nodes already checked' +
        ' for unifability');
      return(true);
    }
  }
  ...
  function copyUnifact0(fs, signature) {
    if (fs) {
      if (!cycleDetect[fs.id]) {
        ...
        if (fs.unifact && (fs.unifact_mark === current_generation)) {
          return(fs.unifact)
        }
        else {
          return(fs);
...
TypedFeatureStructure.prototype.isEqualTo = function(fs) {
  ...
  function isEqualTo0(fs1, fs2) {
    if (!cycleDetect[fs1.id]) {
      ...
      logger.warn('TypedFeatureStructure.isEqualTo: node already compared');
      return(true);
    }
  }
};
  ...
  function completeFS0(fs, signature) {
    if (fs) {
      if (!cycleDetect[fs.id]) {
        ...
        logger.warn('TypedFeatureStructure.completeFS: cycle detected');
        return(fs);
      }
    }
    else {
