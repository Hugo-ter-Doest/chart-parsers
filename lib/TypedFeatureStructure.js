...
// Constructor
function TypedFeatureStructure(type) {
  this.features = {};
  this.type = type;
  this.aux_type = null;
  this.aux_features = {};
...
TypedFeatureStructure.prototype.unify = function(fs, type_lattice) {
  ...
  if (this.unifiable(fs, type_lattice)) {
    ...
    var result = new TypedFeatureStructure(type_lattice.top);
    return(result);
  }
};
...
TypedFeatureStructure.prototype.copyUnifact = function(type_lattice) {
  ...
  if (need_to_copy) {
    if (fs.unifact === null) {
      ...
      fs.unifact = new TypedFeatureStructure(type_lattice.bottom);
      if (fs.aux_type) {
...
TypedFeatureStructure.prototype.copy0 = function(type_lattice) {
  var result = new TypedFeatureStructure(this.type);
  var that = this;
