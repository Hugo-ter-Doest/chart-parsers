...
function unifiableStrings(fs1, fs2) {
...
TypedFeatureStructure.prototype.unifiable = function(fs, signature) {
  ...
  function unifiableListOfCrossRefs(fs1, fs2, signature, stacka, stackb) {
    var unifiable = true;
      ...
      if (unifiable) {
        fs1.listOfCrossRefs.forEach(function (ref, index) {
          if (ref.type === signature.typeLattice.string) {
            ...
            unifiable = unifiable && unifiable0(ref, fs2.listOfCrossRefs[index], signature, stacka, stackb);
          }
        });
      }
      if (unifiable) {
  ...
  function unifiable0(a, b, signature, stacka, stackb) {
    ...
    if (fs1.aux_type === signature.typeLattice.string) {
      return(unifiableStrings(fs1, fs2));
    }
    else {
      if (fs1.aux_type === signature.typeLattice.list) {
        return(unifiableListOfCrossRefs(fs1, fs2, signature, newStacka, newStackb));
      }
      else { // Ordinary node
        ...
        function featureIsUnifiable(feature) {
          ...
          if (fs1.features[feature]) {
            ...
            if ((newStacka.indexOf(fs1.features[feature].id) === -1) ||
              ...
              var unifiable = unifiable0(fs1.features[feature], fs2.features[feature], signature, newStacka, newStackb);
